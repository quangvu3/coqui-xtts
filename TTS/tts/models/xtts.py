...
class Xtts(BaseTTS):
    ...
    def load_safetensors_checkpoint(
        ...
        """
        Loads a safetensor checkpoint from disk and initializes the model's state and tokenizer.

        Args:
            config (dict): The configuration dictionary for the model.
            checkpoint_dir (str, optional): The directory where the checkpoint is stored.
            eval (bool, optional): Whether to set the model to evaluation mode. Defaults to True.
            strict (bool, optional): Whether to strictly enforce that the keys in the checkpoint match the keys in the model. Defaults to True.
            use_deepspeed (bool, optional): Whether to use deepspeed for inference. Defaults to False.
        Returns:
            None
        """
        ...
        speaker_dir = os.path.join(checkpoint_dir, "speakers")

        self.language_manager = LanguageManager(config)
        self.speaker_manager = None
        if os.path.exists(speaker_dir):
            self.speaker_manager = SpeakerManager(speaker_dir)
